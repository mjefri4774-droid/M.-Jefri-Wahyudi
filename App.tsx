...
import { 
  ...
  ChevronLeft, ChevronRight, Menu, AlertTriangle, X, Cpu, CheckCircle
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [currentUser, setCurrentUser] = useState<UserProfile | null>(() => loadData('smp_user', null));
  const [users, setUsers] = useState<UserAccount[]>(() => loadData('smp_users_list', INITIAL_USERS));
  const [students, setStudents] = useState<Student[]>(() => loadData('smp_students', INITIAL_STUDENTS));
  const [attendance, setAttendance] = useState<AttendanceRecord[]>(() => loadData('smp_attendance', []));
  const [schoolProfile, setSchoolProfile] = useState<SchoolProfile>(() => loadData('smp_school_profile', DEFAULT_SCHOOL));
  ...
  const addRecord = useCallback((studentId: string, type: 'IN' | 'OUT') => {
    const now = new Date();
  ...
  }, [schoolProfile.lateTime]);

  const processPhysicalRFID = useCallback((uid: string) => {
    ...
    if (student) {
      ...
      if (lastRecord) {
        ...
        if (nowTime - lastTime < 120000) { 
          setLastPhysicalScan({ name: `${student.name} (Gagal: Cooldown)`, type: 'ERROR' });
          speakFeedback("Tunggu sebentar");
          return;
        }
      }
      ...
      if (type === 'IN') {
        if (timeStr > schoolProfile.lateTime) speakFeedback("Terlambat");
        else speakFeedback("Diterima");
      } else {
        speakFeedback("Hati-hati");
      }
  ...
  }, [students, attendance, schoolProfile.outTime, schoolProfile.lateTime, addRecord]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        if (rfidBuffer.length > 2) processPhysicalRFID(rfidBuffer.trim());
        rfidBuffer = "";
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 overflow-hidden h-screen">
      ...
      {/* Mobile Sidebar */}
      <aside className={`fixed inset-y-0 left-0 w-72 bg-emerald-950 text-white z-[160] md:hidden transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="p-6 flex items-center justify-between border-b border-emerald-900/30">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center overflow-hidden">
              {schoolProfile.logoUrl ? <img src={schoolProfile.logoUrl} className="w-full h-full object-cover" alt="Logo" /> : <Layout className="w-6 h-6" />}
            </div>
            <h1 className="font-bold text-lg truncate">{schoolProfile.appTitle}</h1>
          </div>
          <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-white/10 rounded-lg"><X className="w-6 h-6" /></button>
        </div>
        <nav className="flex-1 px-3 py-6 space-y-1">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl ${activeView === item.id ? 'bg-emerald-600' : 'text-emerald-300/70 hover:bg-white/5'}`}
            >
        ...
        <div className="p-6 border-t border-emerald-900/30">
          <button onClick={() => setShowLogoutConfirm(true)} className="w-full flex items-center gap-4 px-5 py-4 text-emerald-300 hover:text-rose-400 rounded-2xl">
      ...
      {lastPhysicalScan && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[300] animate-in slide-in-from-top-4 w-[90%] max-w-sm">
          <div className={`px-6 py-4 rounded-[2rem] shadow-2xl flex items-center gap-4 border-2 ${
            ...
            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center shrink-0"><Cpu className="w-5 h-5" /></div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center mb-6"><LogOut className="w-8 h-8" /></div>
              <h3 className="text-xl font-bold text-slate-800 mb-2">Konfirmasi Keluar</h3>
              <p className="text-slate-500 text-sm">Apakah Anda yakin ingin mengakhiri sesi ini?</p>
              <div className="flex gap-3 w-full mt-8">
                <button onClick={confirmLogout} className="flex-1 bg-emerald-600 text-white py-3.5 rounded-2xl font-black uppercase text-[10px]">Ya, Keluar</button>
                <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 bg-slate-100 text-slate-600 py-3.5 rounded-2xl font-black uppercase text-[10px]">Batal</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-64'} bg-emerald-950 text-white flex-shrink-0 hidden md:flex flex-col transition-all duration-300 border-r border-emerald-900`}>
        <div className="p-6 flex items-center justify-between border-b border-emerald-900/50">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center shrink-0 overflow-hidden shadow-lg">
              {schoolProfile.logoUrl ? <img src={schoolProfile.logoUrl} className="w-full h-full object-cover" alt="Logo" /> : <Layout className="w-6 h-6" />}
            </div>
            {!isSidebarCollapsed && <h1 className="font-bold text-lg truncate">{schoolProfile.appTitle}</h1>}
          </div>
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className={`p-1.5 hover:bg-white/10 rounded-lg text-emerald-300 ${isSidebarCollapsed ? 'rotate-180' : ''}`}><ChevronLeft className="w-5 h-5" /></button>
        </div>
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-3 px-4'} py-3.5 rounded-2xl transition-all ${activeView === item.id ? 'bg-emerald-600 text-white' : 'text-emerald-300/60 hover:bg-white/5'}`}
            >
              <item.icon className="w-5 h-5 shrink-0" />
        ...
        <div className="p-4 border-t border-emerald-900/50">
          <div className={`px-4 py-3 mb-4 bg-emerald-900/30 rounded-2xl flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-3'}`}>
            ...
            {!isSidebarCollapsed && <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Scanner Aktif</span>}
          </div>
          <button onClick={() => setShowLogoutConfirm(true)} className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-3 px-4'} py-3.5 text-emerald-300 hover:text-rose-400 rounded-2xl transition-all`}>
            <LogOut className="w-5 h-5" />
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-[100] shrink-0">
          <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2.5 text-slate-600 border border-slate-100 rounded-xl"><Menu className="w-6 h-6" /></button>
          
          <div className="flex items-center gap-3">
             <div className="hidden sm:block text-slate-400 text-[10px] font-black uppercase tracking-widest">
               {schoolProfile.name} • {schoolProfile.academicYear}
             </div>
             <div className="h-4 w-[1px] bg-slate-200 hidden sm:block mx-2"></div>
             <div className="flex items-center gap-1.5 text-emerald-600">
               <CheckCircle className="w-3.5 h-3.5" />
               <span className="text-[9px] font-black uppercase">Tersimpan: {lastSync.toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'})}</span>
             </div>
          </div>

          <div className="flex items-center gap-3 md:gap-4">
            <button className="p-2.5 text-slate-400 hover:bg-slate-50 border border-slate-100 rounded-xl"><Bell className="w-5 h-5" /></button>
            <div className="w-10 h-10 rounded-2xl bg-emerald-100 border border-emerald-200 flex items-center justify-center text-emerald-700 font-black text-xs overflow-hidden">
              {currentUser.avatar ? <img src={currentUser.avatar} className="w-full h-full object-cover" alt="Avatar" /> : currentUser.name.charAt(0)}
            </div>
          </div>
        </header>
